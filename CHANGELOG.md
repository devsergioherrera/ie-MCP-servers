...
- 2026-06-05 — Expuesta la BD `INTRANET` del servidor `192.168.50.86` via MCP (read-only, todas las tablas/vistas del schema dbo). Nuevo contenedor `mcp-mssql-intranet` (DAB 1.7.93, puerto 5005, ruta `/intranet/mcp`). `mcp_reader` con `db_datareader` (el LOGIN ya existia en .86 por grants-monitor). Las entidades del `dab-config.json` se generan automaticamente con `gen-config.py` (introspecta INFORMATION_SCHEMA) — DAB no soporta wildcard de tablas. ATENCION: alcance "toda la BD" puede exponer PII (Ley 1273), aceptado explicitamente por el usuario. Grant script en `data-mcp-servers/mssql/grants-intranet.sql`.
